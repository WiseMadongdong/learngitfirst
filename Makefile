...
release_hdr := $(shell sh -c './mkreleasehdr.sh')   #第一句就不懂啦。。好久不写Makefile都忘的差不多了，看了一下这句，release_hdr: 意思应该是要建立一个名字叫release_hdr的目标项目，冒
#号后边的应该是建立目标项目需要的文件或是目标，又看了一下，应该不对，出现了等号应该是定义了一个变量，也就是说 release_hdr: 是一个变量，像定义了一个宏一样，后面的一句是它的值，也就是
#后面会用到这个变量，找找看一下，后面没有。。这是什么意思呢？终于搞懂了，原来意思是后边的变量如果如果定义过就使用，没定义过就不用了。那么右边$()里的又是什么意思呢？明显不是一个变量啊
#原来是这个意思，$后面的shell命令，意思是先执行这个命令，然后把执行的结果赋值给前边的release_hdr变量。要理解这个变量的值，需要看看mkereleasehdr.sh这个脚本的内容。
